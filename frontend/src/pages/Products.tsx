...
export default function Products() {
  ...
  const { data: productsData, isLoading: productsLoading } = useQuery({
    queryKey: ['products', searchTerm, currentCategory, isVeg, isSpicy, maxPrice, sortBy],
    queryFn: async () => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          ...
          <form onSubmit={handleSearch} className="relative w-full md:max-w-md">
            <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
              <Search className="h-5 w-5 text-[var(--saffron-400)]" />
            </div>
            <Input
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search products..."
              className="h-12 rounded-full border-[var(--border-strong)] bg-[var(--bg-elevated)] !pl-14 pr-24 focus:border-[var(--saffron-500)] shadow-[0_0_15px_rgba(245,137,10,0.1)] transition-all"
            />
            <Button
              type="submit"
                ...
                <div className="pt-4 mt-4 border-t border-[var(--border-subtle)]">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-sm font-semibold text-[var(--saffron-400)]">
                      {maxPrice === 2000 ? 'Any' : `INR ${maxPrice}`}
                    </span>
                  </div>
                  <input
                    type="range"
